...
#' @param mean.A Mean effect of 'standard' treatment (i.e. placebo) in a logit scale; this is used for binomial outcome when the risk difference or relative risk is needed; this should be informed from external evidence or can be found by meta-analyzing single proportions; For number needed to treat, we assume that events are "bad". Reversal of sign is needed if the events are "good".
