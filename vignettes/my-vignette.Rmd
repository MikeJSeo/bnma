...
Another important addition to this network package is the ability to add baseline risk. Here we show how to add common treatment x baseline risk interactions (slopes) and independent baseline risk. We can have "common", "independent", "exchangeable" assumption for the baseline slopes and "indepednet" and "exchangeable" assumption on the baseline risk.

```{r}
network <- with(certolizumab, network.data(Outcomes = Outcomes, Treat = Treat, Study = Study, N = N, response = "binomial", Treat.order = Treat.order, baseline = "exchangeable", baseline.risk = "exchangeable"))
result <- network.run(network)
summary(result)
```
