...
## Finding risk difference, and relative risk with Binomial outcomes

We estimate odds ratios using our NMA model, and then we apply these odds ratios to a baseline risk (i.e. risk in placebo), to estimate absolute event rates for all treatments. Using these absolute event rates, we are then able to find number needed to treat, risk difference, and relative risk. In this example, we use metaprop function from meta package to meta-analyze placebo event proportion.

```{r}
#using metaprop (from meta), we estimate placebo event rate
library(meta)
...
prob.pla <- exp(meta.pla$TE.fixed)/(1+exp(meta.pla$TE.fixed))

network <- with(certolizumab, network.data(Outcomes = Outcomes, Treat = Treat, Study = Study, N = N, response = "binomial", A.probability = prob.pla))
result <- network.run(network)
