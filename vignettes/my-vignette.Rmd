...
## Finding number needed to treat, risk difference, and relative risk
...
```{r}
network <- with(certolizumab, network.data(Outcomes = Outcomes, Treat = Treat, Study = Study, N = N, response = "binomial", A.probability = 0.05))
result <- network.run(network)
```
